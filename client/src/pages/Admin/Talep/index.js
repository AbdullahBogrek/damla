...
import { fetchTalepList, deleteTalep } from "../../../Api";
...
function Talep() {
  ...
  const { isLoading, isError, data, error } = useQuery('admin:taleps', fetchTalepList)
  
  const deleteMutation = useMutation(deleteTalep, {
    onSuccess: () => queryClient.invalidateQueries('admin:taleps')
  })
  ...
  return (
    <>
      <div className="container">
        <header className="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-2 my-3 border-bottom shadow p-3 bg-body rounded">
            ...
            <Table dataSource={data} columns={columns} rowKey="_id"/>
              {/* <div className="row px-2 d-flex justify-content-between table-responsive">
                  <table className="table table-striped table-sm">
                        ...
                        <tbody>
                          { 
                              talepler.map((talep, key) => (
                                <tr key={talep._id}>
                                  <td>{key + 1}</td>
                                  <td>{capitalizeFirstWords(talep.title)}</td>
                                  <td>{capitalizeFirstWords(talep.province)}</td>
                                  <td>{capitalizeFirstWords(talep.district)}</td>
                                  <td>{moment(talep.createdAt).format("DD/MM/YYYY")}</td>
                                  <td>
                                          ...
                                          <div className="modal-dialog modal-dialog-centered">
                                              <div className="modal-content rounded-3 shadow">
                                                  <div className="modal-body p-4 text-center">
                                                    ...
                                                    <p className="mb-0">Bu işlem sonucunda oluşturulan yardım talepı her yerden silinecektir. Silinen talep geri getirilemez. </p>
                                                  </div>
                                                  <div className="modal-footer flex-nowrap p-0">
                                                    <Link 
                                                      ...
                                                      onClick={() => {
                                                        deleteMutation.mutate(talep._id, {
                                                          onSuccess: () => {
